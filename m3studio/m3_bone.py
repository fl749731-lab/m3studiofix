...
def _ensure_bone_anim_ids(pose_bone):
    """Auto-initialize M3 animation hex IDs for new bones."""
    shared.m3_data_handles_verify(pose_bone.id_data.pose.bones)
    if pose_bone.bl_handle not in (locker.bone for locker in pose_bone.id_data.m3_bone_id_lockers):
        locker = pose_bone.id_data.m3_bone_id_lockers.add()
        locker.bone = pose_bone.bl_handle
    for prop in bone_anim_props:
        if not pose_bone.get(prop):
            pose_bone[prop] = shared.m3_anim_id_gen()
...
def get_bone_loc_id(self):
    try:
        ...
        _ensure_bone_anim_ids(self)
        return self.get('m3_location_hex_id', '')
...
def get_bone_rot_id(self):
    try:
        ...
        _ensure_bone_anim_ids(self)
        return self.get('m3_rotation_hex_id', '')
...
def get_bone_scl_id(self):
    try:
        ...
        _ensure_bone_anim_ids(self)
        return self.get('m3_scale_hex_id', '')
...
def get_bone_bat_id(self):
    try:
        ...
        _ensure_bone_anim_ids(self)
        return self.get('m3_batching_hex_id', '')
